...
export default function ContactToast({ show, onClose }: ContactToastProps) {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`fixed bottom-6 right-6 z-50 flex items-start gap-4 bg-[#0d1b3e] text-white px-6 py-4 rounded-2xl shadow-2xl max-w-sm w-full transition-all duration-500 ease-out ${show ? "translate-y-0 opacity-100 pointer-events-auto" : "translate-y-8 opacity-0 pointer-events-none"
        }`}
    >
      <div className="flex-shrink-0 mt-0.5 w-8 h-8 rounded-full bg-[#b8974a] flex items-center justify-center">
        <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
